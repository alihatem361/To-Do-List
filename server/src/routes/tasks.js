...
router.get('/:id', (req, res) => {
  const task = db.read().tasks.find((t) => t.id === Number(req.params.id) && ownedBy(t, req.user.id));
...
router.put('/:id', (req, res) => {
  ...
  const task = db.update((data) => {
    const found = data.tasks.find((t) => t.id === Number(req.params.id) && ownedBy(t, req.user.id));
...
router.delete('/:id', (req, res) => {
  const removed = db.update((data) => {
    const index = data.tasks.findIndex((t) => t.id === Number(req.params.id) && ownedBy(t, req.user.id));
